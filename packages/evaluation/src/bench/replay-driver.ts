...
async function replayOne(
  ...
  for (const message of candidate.messages) {
    ...
    // Recording the original decision is enough: the next turn finds the decided
    // approval exactly as the original did. The turn is deliberately not re-sent,
    // which would add a customer message the original conversation never had and
    // misalign every later turn.
...
/**
 * A tool the original run had no decision for is left pending: inventing an
