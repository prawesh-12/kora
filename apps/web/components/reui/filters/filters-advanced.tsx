    ...
    >
      ...
      <div className={CONTENT_BAND_CLASS}>
        <div className={cn(CELL_BOX_CLASS, FIELD_CELL_CLASS)}>
          ...
          >
            <PopoverTrigger asChild disabled={actions.disabled} {...filterReadOnlyProps(actions)}>
              <Button
                variant="outline"
                size={sizes.button}
...
function FilterGroupMenu<V>({
  ...
  return (
    ...
    >
      <DropdownMenuTrigger asChild disabled={actions.disabled} {...filterReadOnlyProps(actions)}>
        <Button
          variant="ghost"
          size={sizes.icon}
...
export function FiltersAdvancedPanel<V, O>({
  ...
  const focusFirstIssue = React.useCallback(() => {
    ...
    const row = Array.from(body.querySelectorAll<HTMLElement>(ROW_SELECTOR)).find(
      (candidate) => candidate.dataset.nodeId === first.nodeId,
    );
    if (!row) return;
    // A group's issue is drawn on the add button that resolves it.
    const column = first.column === 'group' ? 'add' : first.column;
    const target = ownCells(row).find((cell) => cell.getAttribute(CELL_ATTRIBUTE) === column);
    target?.focus();
  }, [issues]);
